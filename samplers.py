    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-2], t_steps[1:-1])):
            ...
            if cfg_scale > 1. and t_cur <= guidance_high and t_cur >= guidance_low:
                ...
                if args.vae_cfg == 'true':
                    d_cur_vae, rest = d_cur[:, :4], d_cur[:, 4:]
                    
                    rest_cond, rest_uncond = rest.chunk(2)
                    
                    d_cur_cond_vae, d_cur_uncond_vae = d_cur_vae.chunk(2)
                    d_cur_vae = d_cur_cond_vae + cfg_scale * (d_cur_cond_vae - d_cur_uncond_vae)
                    
                    d_cur = torch.cat([d_cur_vae, rest_cond], dim=1)
                else:
